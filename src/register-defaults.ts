...
//
// Lives in `src/` (not `src/types/`) deliberately. Perry's module loader
// caches modules by raw import-path string, so two files importing the
// same registry under different relative paths (`./registry` vs
// `./types/registry` vs `../registry`) end up referencing different
// module instances — each with its own private `REGISTRY_OIDS` array.
// On Perry, that means `registerType()` calls from one path never
// surface in `getCodec()` calls made via another path.
//
// Co-locating the registration calls here, in `src/`, makes them use
// the exact same `'./types/registry'` import path that `connection.ts`
// uses. Both paths resolve to the same Perry module instance, so the
// pushes from `registerDefaultCodecs()` are visible to the `getCodec()`
// reads done inside `decodeValue()` when rows arrive from the wire.

import { registerType } from './types/registry';
import * as O from './types/oids';
import {
    boolCodec,
    byteaCodec,
    float4Codec,
...
import { arrayCodec } from './types/codecs/array';
import type { Codec } from './types/registry';
...
export function registerDefaultCodecs(): void {
    ...
    const numericCodec: Codec<Decimal | string | number | bigint> = {
        oid: O.OID_NUMERIC,
        name: 'numeric',
        text: { decode: decodeNumericText, encode: encodeNumericText },
        binary: { decode: decodeNumericBinary, encode: encodeNumericBinary },
    };
    registerType(O.OID_NUMERIC, numericCodec);

    // ── Date / time ─────────────────────────────────────────────────
    registerType<PgDate>(O.OID_DATE, {
        oid: O.OID_DATE, name: 'date',
        text: {
        ...
        binary: {
            ...
            encode: (_v: PgDate): Buffer => { throw new Error('date binary encode not implemented — send as text'); },
        },
    });
    registerType<PgTime>(O.OID_TIME, {
        oid: O.OID_TIME, name: 'time',
        text: {
        ...
        binary: {
            ...
            encode: (_v: PgTime): Buffer => { throw new Error('time binary encode not implemented — send as text'); },
        },
    });
    registerType<PgTime>(O.OID_TIMETZ, {
        oid: O.OID_TIMETZ, name: 'timetz',
        text: {
        ...
        binary: {
            ...
            encode: (_v: PgTime): Buffer => { throw new Error('timetz binary encode not implemented — send as text'); },
        },
    });
    registerType<PgTimestamp>(O.OID_TIMESTAMP, {
        oid: O.OID_TIMESTAMP, name: 'timestamp',
        text: {
        ...
        binary: {
            ...
            encode: (_v: PgTimestamp): Buffer => { throw new Error('timestamp binary encode not implemented — send as text'); },
        },
    });
    registerType<PgTimestamp>(O.OID_TIMESTAMPTZ, {
        oid: O.OID_TIMESTAMPTZ, name: 'timestamptz',
        text: {
        ...
        binary: {
            ...
            encode: (_v: PgTimestamp): Buffer => { throw new Error('timestamptz binary encode not implemented — send as text'); },
        },
    });
    registerType<PgInterval>(O.OID_INTERVAL, {
        oid: O.OID_INTERVAL, name: 'interval',
        text: {
        ...
        binary: {
            ...
            encode: (_v: PgInterval): Buffer => { throw new Error('interval binary encode not implemented — send as text'); },
        },
    });
...
// Auto-register on module import for Node/Bun consumers — they execute
// top-level module code so this runs once at startup. Perry's AOT loader
// skips top-level bodies, which is why `connect()` also calls the
// function explicitly. The internal `registered` flag makes both paths
// idempotent.
